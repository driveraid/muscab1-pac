#[doc = "Register `BRGINTSTAT` reader"]
pub type R = crate::R<BrgintstatSpec>;
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        write!(f, "{}", self.bits())
    }
}
#[doc = "Bridge Buffer Error Interrupt Status\n\nYou can [`read`](crate::Reg::read) this register and get [`brgintstat::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct BrgintstatSpec;
impl crate::RegisterSpec for BrgintstatSpec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`brgintstat::R`](R) reader structure"]
impl crate::Readable for BrgintstatSpec {}
#[doc = "`reset()` method sets BRGINTSTAT to value 0"]
impl crate::Resettable for BrgintstatSpec {
    const RESET_VALUE: u32 = 0;
}
